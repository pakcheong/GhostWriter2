...
(async () => {
  const result = await autoGenerateArticlesFromTopics({
    topics: {
      domain: 'Malaysia personal finance 2025 (EPF/KWSP, ASB, budgeting, inflation, takaful, PTPTN, side income, digital banking, e-wallet adoption, retirement planning)',
    ...
    article: {
      ...
      onArticle(a: any) { logArticle('auto-personal-finance-my', a); }
    } as any,
    // Generate top 4 viable topics (adjust as needed). Omit or set -1 to do all.
    count: 4,
    concurrency: 2,
    verbose: true
  });
