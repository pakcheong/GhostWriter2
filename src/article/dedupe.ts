...
interface DedupeOptions { verbose?: boolean }

function normalizeHeading(h: string): string {
  return h.replace(/[。．.]+$/u, '').replace(/\s+/g, ' ').trim().toLowerCase();
}
...
export function dedupeOutline(outline: OutlineResult, opts: DedupeOptions = {}): OutlineResult {
  ...
  for (const section of outline.outline) {
    ...
    if (!seen.has(key)) { order.push(key); seen.set(key, { heading: section.heading, subheadings: [] }); }
    ...
    for (const sh of section.subheadings) {
      ...
      if (!skey) continue;
      if (!seenSubs.has(skey)) { bucket.subheadings.push(sh); seenSubs.add(skey); }
    }
  }
