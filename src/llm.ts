...
export async function safeGenerateText(
  args: any,
  context: { provider: string; model: string; phase: string }
) {
  try {
          ...
          for (const line of lines) {
            ...
            if (trimmed.startsWith('data:')) {
              ...
              try {
                ...
                const delta =
                  deltaObj.choices?.[0]?.delta?.content || deltaObj.choices?.[0]?.message?.content;
    ...
    if (status === 404) {
      ...
      if (context.provider === 'deepseek')
        hints.push('Hints: Ensure base URL includes /v1', 'Check model name (deepseek-chat).');
      else if (context.provider === 'lmstudio')
        hints.push(
          'Hints: Ensure LM Studio server running.',
