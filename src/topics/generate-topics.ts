...
export async function generateTopics(options: GenerateTopicsOptions): Promise<GenerateTopicsResult> {
  ...
  const prompt = 'You are an assistant that proposes currently compelling, possibly trending ' + domain + ' article topics.\n' +
    langLine +
  ...
  if (/^```/m.test(raw)) raw = raw.replace(/^```(?:json)?/i, '').replace(/```$/i, '').trim();
  try {
    parsed = JSON.parse(raw); if (!Array.isArray(parsed)) parsed = [];
  } catch {
    const objectMatches = raw.match(/\{[\s\S]*?\}/g) || [];
    if (objectMatches.length > 2) {
      const objs: any[] = [];
      for (const om of objectMatches) {
        try { objs.push(JSON.parse(om)); } catch { /* ignore fragment */ }
      }
      parsed = objs;
    }
    if (parsed.length === 0) {
  ...
  const norm: TopicCandidate[] = parsed.map(o => {
    const title = String(o.title || '').trim();
    const rf = Array.isArray(o.riskFlags) ? o.riskFlags.map((r: any) => String(r)) : [];
    return { title, rationale: o.rationale ? String(o.rationale) : undefined, confidence: typeof o.confidence === 'number' ? o.confidence : undefined, riskFlags: rf, sourceType: 'llm' } as TopicCandidate;
  }).filter(c => c.title.length > 0 && c.title.length < 120);

  const seen = new Set<string>();
  const deduped = norm.filter(c => { const k = c.title.toLowerCase(); if (seen.has(k)) return false; seen.add(k); return true; });
  ...
  const toRegex = (r: string | RegExp | undefined) => {
    ...
    try { return new RegExp(r, 'i'); } catch { return undefined; }
  };
  ...
  if (filtered.length === 0) filtered = deduped; // fallback if over-filtered
  ...
  const baseList: TopicCandidate[] = filtered.length > 0 ? filtered : deduped.length > 0 ? deduped : Array.from({ length: limit }, (_, i) => ({ title: `Topic Idea ${i + 1}`, sourceType: 'llm' as const }));
  const scored = baseList.map(c => ({ c, score: (typeof c.confidence === 'number' ? c.confidence : 0.55) - ((c.riskFlags?.length || 0) * 0.05) }));
  ...
  const priceIn = pricing.in ?? 0; const priceOut = pricing.out ?? 0;
  ...
  const result: GenerateTopicsResult = {
    ...
    cost: pricing.found ? { generation: costGeneration, total: totalCost, priceInPerK: pricing.in, priceOutPerK: pricing.out } : undefined,
  ...
  if (printUsage) {
    ...
    if (pricing.found) console.log(`Pricing (per 1K tokens): input=${priceIn != null ? `$${priceIn}` : 'n/a'}  output=${priceOut != null ? `$${priceOut}` : 'n/a'} (source: ${pricing.source})`); else console.log('Pricing (per 1K tokens): n/a');
    const table = new Table({ head: ['Phase', 'Prompt', 'Completion', 'Total', 'Cost'], colWidths: [12, 12, 12, 12, 16], style: { head: [], border: [] } });
    const fmt = (n?: number) => (typeof n === 'number' ? formatUSD(n) : 'n/a');
    table.push(['Generation', `${usageGeneration.promptTokens}`, `${usageGeneration.completionTokens}`, `${usageGeneration.totalTokens}`, fmt(costGeneration)]);
    table.push(['TOTAL', `${totalUsage.promptTokens}`, `${totalUsage.completionTokens}`, `${totalUsage.totalTokens}`, fmt(totalCost)]);
  ...
  if (typeof onTopics === 'function') {
    try { await onTopics(result); } catch (err) { if (verbose) console.warn('[onTopics] callback error:', err); }
  }
  return result;
}
