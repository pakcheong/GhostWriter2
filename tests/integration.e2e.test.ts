...
__setGenerateTextImpl(async (req: any) => {
  ...
  if (content.includes('You are a precise SEO writer')) {
    return { text: 'Some **mock** body with [image]alt[/image]', usage: { promptTokens: 3, completionTokens: 7, totalTokens: 10 } };
  }
  ...
  if (content.includes('"outline"') || content.includes('STRICT JSON') || content.includes('"slug"')) {
    return { text: JSON.stringify({
      title: 'E2E Title', description: 'E2E description meta.', slug: 'e2e-title', tags: ['tag1','tag2'], categories: ['cat1'], outline: [ { heading: 'Intro', subheadings: ['What','Why'] }, { heading: 'Deep Dive', subheadings: ['How','Next'] } ]
